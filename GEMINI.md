...
1. User input → `generate_story()` → Gemini LLM → structured JSON with characters/setting/plot
2. Characters + Setting + Plot → `develope_story()` → Gemini LLM → JSON array of scenes with image/video prompts
...
4. Scene images → uploaded to GCS → Veo 2.0/3.0 → videos downloaded to `tmp/default/`
5. Individual videos → `merge_videos_moviepy()` → `tmp/default/merged_video.mp4`
